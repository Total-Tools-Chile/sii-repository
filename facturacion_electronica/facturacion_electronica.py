...
def procesar_documento(vals):
    ...
    for docs in vals.get('Documento'):
        ...
        if caf_file:
            try:
                caf_mgr = Caf(caf_file)
            except Exception:
                caf_mgr = None
        for docData in docs["documentos"]:
            # Si no viene Folio (o viene 0), lo asignamos automáticamente usando el CAF
            # y persistimos el último folio usado para no repetir.
            try:
                iddoc = docData.setdefault("Encabezado", {}).setdefault("IdDoc", {})
                folio_val = iddoc.get("Folio", 0) or 0
                if int(folio_val) <= 0 and caf_mgr:
                    iddoc["Folio"] = caf_mgr.next_folio(TipoDTE, rut_emisor=vals.get("Emisor", {}).get("RUTEmisor"))
            except Exception:
                pass
            docu = Documento(
                            docData,
...
def consumo_folios(vals):
    ...
    try:
        ...
        msg = "Error en envío CF: %s" % str(e)
        _logger.warning(msg, exc_info=True)
        resultado['errors'] = msg
    del envios[key]
    return resultado
...
def consumo_folios_resumen(data, folios=False):
    ...
    if folios:
        ...
        for row in folios:
            if i > 0:
                if row[0] not in fechas:
                    fechas[row[0]] = ConsumoFolios({})
                if 'Correlativo' in data:
                    fechas[row[0]].Correlativo = data['Correlativo']
                if 'SecEnvio' in data:
                    fechas[row[0]].SecEnvio = data['SecEnvio']
                docu = Documento({
        ...
        for r in data['Documentos']:
            for d in r['documentos']:
                ...
                if fecha not in fechas:
                    fechas[fecha] = ConsumoFolios({})
                if 'Correlativo' in data:
                    fechas[fecha].Correlativo = data['Correlativo']
                if 'SecEnvio' in data:
                    fechas[fecha].SecEnvio = data['SecEnvio']
        ...
        if 'Correlativo' in data:
            cf.Correlativo = data['Correlativo']
        if 'SecEnvio' in data:
            cf.SecEnvio = data['SecEnvio']
        envio = Envio({
            "Emisor": data["Emisor"],
...
def consulta_estado_envio(vals):
    firma = Firma(vals["firma_electronica"])
    emisor = Emisor(vals["Emisor"])
    token = vals.get("token") or vals.get("sii_token")
    conex = Conexion(emisor, firma, vals.get('api', False), token=token)
    if vals.get('cesion'):
